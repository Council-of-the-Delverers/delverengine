...
public class ParticleSphereEmitter extends Entity {
    public ParticleSphereEmitter() { hidden = true; spriteAtlas = "editor"; tex = 14; isSolid = false; }
    ...
    @EditorProperty( group="Sphere Properties" )
    ...
    public void tick(Level level, float delta)
    {
        ...
        if(spawntimer >= particleSpawnInterval) {
            ...
            for(int i = 0; i < particleSpawnCount + rand; i++) {
                ...
                float radius; //this is the radius of the sphere

                //now, we need three random numbers between 0 and 2 * PI (set to -0.5, 4, PI as default)
                float xs = (Game.rand.nextFloat()-0.5F) * 4 * PI;
                float ys = (Game.rand.nextFloat()-0.5F) * 4 * PI;
                float zs = (Game.rand.nextFloat()-0.5F) * 4 * PI;

                //we need their squared value
                float squaredx = xs*xs;
                float squaredy = ys*ys;
                float squaredz = zs*zs;

                //now, we need their sum
                float sum = squaredx + squaredy + squaredz;

                //the square root of this sum
                float sqrtSum = (float) Math.sqrt(sum);

                //and finally, we invert it
                float randomPos = 1/sqrtSum;

                //get the position ON a sphere of radius 1
                float xOne = xs * randomPos;
                float yOne = ys * randomPos;
                float zOne = zs * randomPos;

                //we now have a random xyz on a sphere of radius 1. If we randomize the size of this sphere, we get a random point INSIDE our sphere
                float randomRadius = Game.rand.nextFloat() * sphereRadius;

                p.x = x+xOne * randomRadius;
                p.y = y+yOne * randomRadius;
                p.z = z+zOne * randomRadius;
